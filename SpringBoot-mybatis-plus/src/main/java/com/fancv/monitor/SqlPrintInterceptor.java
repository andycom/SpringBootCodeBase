...
public class SqlPrintInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        String s = invocation.getArgs()[0].toString();
    ...
    private final String ARG_PRE_FIX = "param";
...
//                MapperMethod.ParamMap
            ...
            } else if (MapperMethod.ParamMap.class.isAssignableFrom(parameterObjectClass)) {
                Set set = ((MapperMethod.ParamMap) parameterObject).keySet();
                int size = set.size() / 2;//?的个数
                HashMap<String, String> map = new HashMap<>();
                for (int i = 1; i <= size; i++) {
                    String placeHolder = ARG_PRE_FIX + i;

                    sql = sql.replaceFirst("\\?", "#{" + ARG_PRE_FIX + i + "}");
                    Object o = ((MapperMethod.ParamMap) parameterObject).get((Object) placeHolder);
                    map.put(placeHolder, o.toString());
    ...
    /**
     * 203      * 是否DefaultSqlSession的内部类StrictMap
     * 204
     */
